...
Env:
  ...
  UI_REVIEW_EXPECT_TITLE     Expected title substring (default: CryptoRebate)
...
url="${UI_REVIEW_URL:-}"
expect_title="${UI_REVIEW_EXPECT_TITLE:-CryptoRebate}"
...
if [[ -z "$artifact_dir" ]]; then
  artifact_dir="$(mktemp -d "${TMPDIR:-/tmp}/cryptorebate-ui-review.XXXXXX")"
else
...
session="cryptorebate-ui-review-$$"
cleanup() {
